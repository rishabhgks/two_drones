...
#include <cmath>
#include "two_drones/beam_forming_node.h"
...
int main(int argc, char **argv)
{
  ...
  // %Tag(INIT)%
  ros::init(argc, argv, "beam_forming_node");
  // %EndTag(INIT)%
...
void Drone_Mission::thresholdCallback(const std_msgs::Float64ConstPtr& msg) {
    threshold = *msg;
}
...
void drone1_gps_callback(const sensor_msgs::NavSatFix::ConstPtr& msg) {
  ...
  switch(drone1->state) {
    ...
    case 2:
      ...
      break;
    case 3:
      if((drone1->finished == false) || (drone1->threshold.data < 0.5)) {
        step(*drone1);
      } else {
        drone1->state = 4;
