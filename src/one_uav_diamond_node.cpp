...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ROS_INFO("Current state: %d | Current position: %f %f %f | Current Time: %f", drone1->state, drone1->target.x, drone1->target.y, drone1->target.z, (ros::Time::now().toSec() - drone1->time));
    if((ros::Time::now().toSec() - drone1->time) > 20 && (ros::Time::now().toSec() - drone1->time) < 30 && count < 2) {
      drone1->state = 1;
      drone1->setTarget(drone1->mean_start_gps_x, drone1->mean_start_gps_y, drone1->mean_start_gps_alt + 0.5, 0.0);
      ROS_INFO("Trying to control now");
      if(count == 0)
      {
        center.x = (drone1->curr_odom_.pose.pose.position.x);
        center.y = (drone1->curr_odom_.pose.pose.position.y);
  ...
  switch(drone1->state) {
    ...
    case 1:
      if(drone1->finished == false) {
        ...
        drone1->state = 3;
        drone1->setTarget(drone1->waypoints[0]); // drone1->target.x = -3;
        // drone1->target.y = 4.5;
        // drone1->target.z = -5;
