...
int main(int argc, char **argv)
{
  ...
  // %Tag(INIT)%
  ros::init(argc, argv, "two_uav_diamond_node");
  // %EndTag(INIT)%
  ...
  while (ros::ok())
  {
    ...
    if((ros::Time::now().toSec() - drone1->time) > 20 && (ros::Time::now().toSec() - drone1->time) < 30 && count < 2) {
      ...
      if(count == 0)
      {
        center.x = (drone1->mean_start_gps_x + drone2->mean_start_gps_x)/2;
        ...
        calculate_waypoints(center, drone2->waypoints, 1);
      }
      ++count;
  ...
  switch(drone1->state) {
    ...
    case 1:
      if(drone1->finished == false) {
        ...
        if(drone1->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone1->sync)
            drone2->sync = true;
          drone1->state = 3;
          drone1->setTarget(drone1->waypoints[0]);
      ...
      if(drone1->finished == false) {
        ...
        if(drone1->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone1->sync)
            drone2->sync = true;
          drone1->state = 4;
          drone1->setTarget(drone1->waypoints[1]);
          drone1->reached_goal_ = false;
          drone1->finished = false;
          drone1->sync = false;
      ...
      if(drone1->finished == false) {
        ...
        if(drone1->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone1->sync)
            drone2->sync = true;
          drone1->state = 5;
          drone1->setTarget(drone1->waypoints[2]);
          drone1->reached_goal_ = false;
          drone1->finished = false;
          drone1->sync = false;
      ...
      if(drone1->finished == false) {
        ...
        if(drone1->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone1->sync)
            drone2->sync = true;
          drone1->state = 6;
          drone1->setTarget(drone1->waypoints[3]);
          drone1->reached_goal_ = false;
          drone1->finished = false;
          drone1->sync = false;
      ...
      if(drone1->finished == false) {
        ...
        if(drone1->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone1->sync)
            drone2->sync = true;
          drone1->state = 7;
          drone1->setTarget(drone1->waypoints[4]);
          drone1->reached_goal_ = false;
          drone1->finished = false;
          drone1->sync = false;
          ROS_INFO("Done with route 5");
        }
      }
      break;
    case 7:
      if(drone1->finished == false) {
        ...
        if(drone1->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone1->sync)
            drone2->sync = true;
          drone1->state = 8;
          drone1->setTarget(drone1->waypoints[5]);
          drone1->reached_goal_ = false;
          drone1->finished = false;
          drone1->sync = false;
          ROS_INFO("Done with route 6");
        }
      }
      break;
    case 8:
    ...
    case 10:
      if(drone1->finished == false) {
        ...
        drone1->state = 11;
        drone1->setTarget(drone1->mean_start_gps_x - 1, drone1->mean_start_gps_y, drone1->mean_start_gps_alt + 1.5, 0);
        drone1->reached_goal_ = false;
        drone1->finished = false;
        ros::Duration(5.0).sleep();
        ROS_INFO("Done with route 9");
      }
      break;
    case 11:
      if(drone1->finished == false) {
        ...
        drone1->state = 12;
        drone1->setTarget(drone1->waypoints[8]);
        drone1->reached_goal_ = false;
        drone1->finished = false;
        ros::Duration(5.0).sleep();
        ROS_INFO("Done with route 10");
      }
      break;
    case 12:
      if(drone1->finished == false) {
        ...
        drone1->state = 13;
        drone1->setTarget(drone1->waypoints[9]);
        drone1->reached_goal_ = false;
        drone1->finished = false;
        ROS_INFO("Done with route 10");
      }
    ...
    case 13:
      if(drone1->finished == false) {
        ...
        drone1->state = 14;
        drone1->setTarget(drone1->waypoints[10]);
        drone1->reached_goal_ = false;
        drone1->finished = false;
        ROS_INFO("Done with route 10");
      }
    ...
    case 14:
      if(drone1->finished == false) {
        ...
        drone1->state = 2;
        drone1->setTarget(drone1->waypoints[11]);
        drone1->reached_goal_ = false;
        drone1->finished = false;
        ROS_INFO("Done with route 10");
      }
  ...
  switch(drone2->state) {
    ...
    case 1:
      if(drone2->finished == false) {
        ...
        if(drone2->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone2->sync)
            drone1->sync = true;
          drone2->state = 3;
          drone2->setTarget(drone2->waypoints[0]); 
      ...
      if(drone2->finished == false) {
        ...
        if(drone2->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone2->sync)
            drone1->sync = true;
          drone2->state = 4;
          drone2->setTarget(drone2->waypoints[1]);
          drone2->reached_goal_ = false;
          drone2->finished = false;
          ROS_INFO("Done with route 2");
          drone2->sync = false;
      ...
      if(drone2->finished == false) {
        ...
        if(drone2->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone2->sync)
            drone1->sync = true;
          drone2->state = 5;
          drone2->setTarget(drone2->waypoints[2]);
          drone2->reached_goal_ = false;
          drone2->finished = false;
          ROS_INFO("Done with route 3");
          drone2->sync = false;
      ...
      if(drone2->finished == false) {
        ...
        if(drone2->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone2->sync)
            drone1->sync = true;
          drone2->state = 6;
          drone2->setTarget(drone2->waypoints[3]);
          drone2->reached_goal_ = false;
          drone2->finished = false;
          ROS_INFO("Done with route 4");
          drone2->sync = false;
      ...
      if(drone2->finished == false) {
        ...
        if(drone2->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone2->sync)
            drone1->sync = true;
          drone2->state = 7;
          drone2->setTarget(drone2->waypoints[4]);
          drone2->reached_goal_ = false;
          drone2->finished = false;
          ROS_INFO("Done with route 5");
          drone2->sync = false;
      ...
      if(drone2->finished == false) {
        ...
        if(drone2->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone2->sync)
            drone1->sync = true;
          drone2->state = 8;
          drone2->setTarget(drone2->waypoints[5]);
          drone2->reached_goal_ = false;
          drone2->finished = false;
          ROS_INFO("Done with route 6");
          drone2->sync = false;
      ...
      if(drone2->finished == false) {
        ...
        if(drone2->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone2->sync)
            drone1->sync = true;
          drone2->state = 9;
          drone2->setTarget(drone2->waypoints[6]);
          drone2->reached_goal_ = false;
          drone2->finished = false;
          ros::Duration(5.0).sleep();
          ROS_INFO("Done with route 7");
      ...
      if(drone2->finished == false) {
        ...
        if(drone2->sync || (drone2->finished && drone2->reached_goal_))
        {
          if(!drone2->sync)
            drone1->sync = true;
          drone2->state = 2;
          drone2->setTarget(drone2->waypoints[7]);
          drone2->reached_goal_ = false;
          drone2->finished = false;
          ros::Duration(5.0).sleep();
          ROS_INFO("Done with route 8");
    ...
    case 10:
      if(drone2->finished == false) {
        ...
        drone2->state = 11;
        drone2->setTarget(drone2->mean_start_gps_x - 1, drone2->mean_start_gps_y, drone2->mean_start_gps_alt + 1.5, 0);
        drone2->reached_goal_ = false;
        drone2->finished = false;
        ros::Duration(5.0).sleep();
        ROS_INFO("Done with route 9");
    ...
    case 11:
      if(drone2->finished == false) {
        ...
        drone2->state = 12;
        drone2->setTarget(drone2->waypoints[8]);
        drone2->reached_goal_ = false;
        drone2->finished = false;
        ros::Duration(5.0).sleep();
        ROS_INFO("Done with route 10");
    ...
    case 12:
      if(drone2->finished == false) {
        ...
        drone2->state = 13;
        drone2->setTarget(drone2->waypoints[9]);
        drone2->reached_goal_ = false;
        drone2->finished = false;
        ROS_INFO("Done with route 10");
      }
    ...
    case 13:
      if(drone2->finished == false) {
        ...
        drone2->state = 14;
        drone2->setTarget(drone2->waypoints[10]);
        drone2->reached_goal_ = false;
        drone2->finished = false;
        ROS_INFO("Done with route 10");
      }
    ...
    case 14:
      if(drone2->finished == false) {
        ...
        drone2->state = 2;
        drone2->setTarget(drone2->waypoints[11]);
        drone2->reached_goal_ = false;
        drone2->finished = false;
        ROS_INFO("Done with route 10");
      }
